...
const SELECTED_CHANNEL_KEY = "widgetYouTubeLiveMusicSelectedChannel";
const FAVORITE_CHANNELS_KEY = "widgetYouTubeLiveMusicFavorites";
const VOLUME_KEY = "widgetYouTubeLiveMusicVolume";
const WIDGET_VISIBILITY_KEY = "widgetVisibility";
const WIDGET_VERSION = "1.0.0";
const CHANNEL_PAGE_SIZE = 50;

/* eslint-disable @next/next/no-img-element */
export default function WidgetDraggableYouTubeLiveMusic() {
  const WIDGET_ID = "youtubelivemusic";
  ...
  const {
    ...
    selectedChannelSlug
      ? ["youtube-live-music-channel", selectedChannelSlug]
      : null,
  ...
  >(
    favorites.length > 0
      ? ["youtube-live-music-favorites", ...favorites]
      : null,
    async ([, ...slugs]) => {
      const channels = await Promise.all(
        slugs.map((slug) => fetchYoutubeLiveMusicChannel({ slug })),
      );
  ...
  const groupedChannels = useMemo(
    () => groupYoutubeLiveMusicChannelsByCategory(dialogChannels),
    [dialogChannels],
  );
    ...
    const hydrate = async () => {
      try {
        ...
        if (savedChannelSlug) {
          ...
          const fallbackCollection = await fetchYoutubeLiveMusicCollection({
            page: 1,
            limit: 1,
          });
          if (!isCancelled)
  ...
  useEffect(() => {
    ...
    const playerId = "yt-live-music-player";
  ...
  const loadDialogChannels = useCallback(
    async (page: number, mode: "replace" | "append") => {
      ...
      setDialogError(null);
      try {
        const collection = await fetchYoutubeLiveMusicCollection({
          page,
                ...
                !selectedChannel && (
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="flex flex-col items-center gap-2 text-muted-foreground">
                      ...
                      <span className="text-xs">Loading live music</span>
                    </div>
                  </div>
                )}
              {!selectedChannelError &&
      ...
      <Dialog open={channelPickerOpen} onOpenChange={setChannelPickerOpen}>
        ...
        >
          <DialogHeader className="sr-only">
            ...
            <DialogDescription>
              Search and choose a live YouTube music channel.
            </DialogDescription>
          </DialogHeader>
          <Command
            shouldFilter={false}
