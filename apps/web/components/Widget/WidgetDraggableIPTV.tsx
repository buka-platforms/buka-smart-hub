 ...
 const groupedChannels = iptvChannels.reduce((acc, channel) => {
...
const countries = [...new Set(iptvChannels.map((c) => c.country).filter(Boolean))].sort() as string[];
 ...
 export default function WidgetDraggableIPTV() {
   ...
   useEffect(() => {
     ...
     if (isDragging) {
       ...
       if (prevPlayerPointerRef.current !== null) el.style.pointerEvents = prevPlayerPointerRef.current;
   ...
   const toggleFavorite = useCallback(() => {
     ...
     setFavorites((prev) => {
       ...
       try {
         localStorage.setItem(FAVORITE_CHANNELS_KEY, JSON.stringify(newFavorites));
       } catch {}
       return newFavorites;
     });
   }, [selectedChannel]);
   ...
   const filteredChannels = useMemo(() => {
     ...
     for (const [category, channels] of Object.entries(groupedChannels)) {
       const categoryChannels = channels.filter((c) => c.country === countryFilter);
       if (categoryChannels.length > 0) filtered[category] = categoryChannels;
     }
     return filtered;
   }, [countryFilter]);

   const favoriteChannels = useMemo(() => iptvChannels.filter((c) => favorites.includes(c.id)), [favorites]);
   ...
   const toggleFullscreen = useCallback(() => {
    ...
    const webkitFs = (video as unknown as { webkitRequestFullscreen?: () => Promise<void> | void }).webkitRequestFullscreen;
    const msFs = (video as unknown as { msRequestFullscreen?: () => Promise<void> | void }).msRequestFullscreen;
   ...
   const isFavorite = selectedChannel ? favorites.includes(selectedChannel.id) : false;
       ...
       >
         <div className="relative flex w-full flex-col">
                 ...
                 <DropdownMenuContent align="end" sideOffset={6} className="min-w-44">
                   <DropdownMenuItem
                     ...
                     onSelect={(e) => {
                       ...
                       try {
                         localStorage.setItem(WIDGET_VISIBILITY_KEY, JSON.stringify({ ...visibility, [WIDGET_ID]: false }));
                       } catch {}
                     }}
                   >
                     Hide widget
                   </DropdownMenuItem>
                   <DropdownMenuItem onSelect={toggleFavorite} className="cursor-pointer gap-2">
                     {isFavorite ? "Remove from favorites" : "Add to favorites"}
                   </DropdownMenuItem>
                   {selectedChannel && (
                     <DropdownMenuItem asChild className="cursor-pointer gap-2">
                       <Link href={`/tv/${selectedChannel.id}`}>Open channel page</Link>
                     </DropdownMenuItem>
                   )}
                   <DropdownMenuItem asChild className="cursor-pointer gap-2">
           ...
           <div className="flex w-full flex-col">
             ...
             <div className="flex items-center gap-2 border-b border-white/10 px-3 py-2">
               ...
               {selectedChannel?.logo_url && (
                 <div className="relative h-6 w-6 shrink-0 overflow-hidden rounded bg-white/10">
                   <img src={selectedChannel.logo_url} alt={selectedChannel.name} className="h-full w-full object-contain" loading="lazy" draggable={false} />
                 </div>
               )}

               {/* Channel Info */}
               <div className="flex min-w-0 flex-1 flex-col">
                 <span className="truncate text-xs font-semibold text-white">{selectedChannel?.name || "Select Channel"}</span>
               ...
               <Popover open={channelPickerOpen} onOpenChange={setChannelPickerOpen}>
                 <PopoverTrigger asChild>
                   <button className="flex h-7 cursor-pointer items-center gap-1 rounded-md border border-white/10 bg-white/5 px-2 text-[10px] font-medium text-white/70 transition-colors hover:bg-white/10 hover:text-white" title="Change channel">
                     <Tv className="h-3 w-3" />
                     <ChevronDown className="h-3 w-3" />
                   </button>
                 </PopoverTrigger>
                 <PopoverContent align="end" sideOffset={8} className="w-96 rounded-lg border border-white/10 bg-[#0c0c10]/95 p-0 shadow-2xl backdrop-blur-xl">
                   <Command className="border-0 bg-transparent text-white **:[[cmdk-input-wrapper]]:flex-1 **:[[cmdk-input-wrapper]]:border-0 **:[[cmdk-input-wrapper]]:px-0">
                     <div className="flex items-center gap-2 border-b border-white/10 p-2">
                       <CommandInput placeholder="Search channels..." className="h-8 flex-1 text-sm text-white placeholder:text-white/40" />
                       <Popover>
                         <PopoverTrigger asChild>
                           <button
                         ...
                         <PopoverContent align="end" className="max-h-60 w-48 overflow-y-auto rounded-md border border-white/10 bg-black/95 p-1 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-white/20 [&::-webkit-scrollbar-thumb]:hover:bg-white/30 [&::-webkit-scrollbar-track]:rounded-full [&::-webkit-scrollbar-track]:bg-white/5">
                           <button onClick={() => setCountryFilter(null)} className={`flex w-full cursor-pointer items-center gap-2 rounded px-2 py-1.5 text-left text-xs transition-colors hover:bg-white/10 ${!countryFilter ? "bg-purple-500/20 text-purple-400" : "text-white/70"}`}>
                             All Countries
                           </button>
                           {countries.map((country) => (
                             <button key={country} onClick={() => setCountryFilter(country)} className={`flex w-full cursor-pointer items-center gap-2 rounded px-2 py-1.5 text-left text-xs transition-colors hover:bg-white/10 ${countryFilter === country ? "bg-purple-500/20 text-purple-400" : "text-white/70"}`}>
                     ...
                     <CommandList className="max-h-72 overflow-y-auto bg-transparent [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-white/20 [&::-webkit-scrollbar-thumb]:hover:bg-white/30 [&::-webkit-scrollbar-track]:rounded-full [&::-webkit-scrollbar-track]:bg-white/5">
                       <CommandEmpty className="px-3 py-2 text-xs text-white/60">No channels found.</CommandEmpty>

                       {favoriteChannels.length > 0 && (
                         <CommandGroup heading={<span className="text-[10px] font-semibold tracking-wide text-white/50 uppercase">Favorites</span>}>
                           {favoriteChannels.map((channel) => (
                             <CommandItem key={channel.id} value={`${channel.id}`} onSelect={() => selectChannel(channel)} className={commandItemClass}>
                               <div className="flex w-full items-center gap-3">
                                 {channel.logo_url && (
                                   <div className="flex h-8 w-8 shrink-0 items-center justify-center overflow-hidden rounded-md border border-white/10 bg-white/5 p-1">
                                     <img src={channel.logo_url} alt={channel.name} className="h-full w-full object-contain" draggable={false} />
                                   </div>
                                 )}
                                 <div className="flex min-w-0 flex-1 flex-col">
                                   <span className="truncate text-[13px] font-medium text-white">{channel.name}</span>
                                   <span className="truncate text-[11px] text-white/50">{channel.country}</span>
                                 </div>
                       ...
                       {sortedCategories.map((category) => {
                         ...
                         return (
                           <CommandGroup key={category} heading={<span className="text-[10px] font-semibold tracking-wide text-white/50 uppercase">{category}</span>}>
                             {channels.map((channel) => (
                               <CommandItem key={channel.id} value={`${channel.id}`} onSelect={() => selectChannel(channel)} className={commandItemClass}>
                                 <div className="flex w-full items-center gap-3">
                                   ...
                                   {favorites.includes(channel.id) && <Heart className="h-3.5 w-3.5 text-pink-400" fill="currentColor" />}
                                 </div>
                               </CommandItem>
                             ))}
                           </CommandGroup>
             ...
             {/* Video Player Area */}
             <div ref={playerRef} className="relative aspect-video overflow-hidden bg-black">
             ...
             <div className="flex items-center gap-2 border-t border-white/10 px-3 py-2">
               <button onClick={togglePlay} className="flex h-8 w-8 cursor-pointer items-center justify-center rounded-full border border-white/10 bg-white/10 text-white transition-colors hover:bg-white/20" title={isPlaying ? "Pause" : "Play"}>
                 {isPlaying ? <Pause className="h-4 w-4" fill="currentColor" /> : <PlayIcon className="h-4 w-4" fill="currentColor" />}
               </button>

               <Popover>
                 <PopoverTrigger asChild>
                   <button className="flex h-8 w-8 cursor-pointer items-center justify-center rounded-full border border-white/10 bg-white/10 text-white transition-colors hover:bg-white/20" title="Volume">
                     {volume === 0 ? <VolumeX className="h-4 w-4" /> : volume < 50 ? <Volume1 className="h-4 w-4" /> : <Volume2 className="h-4 w-4" />}
                   </button>
                 </PopoverTrigger>
                 <PopoverContent align="start" sideOffset={6} className="flex w-32 flex-col gap-2 rounded-md border border-white/10 bg-black/90 p-3 shadow-lg">
                   <div className="flex items-center justify-between text-[11px] font-semibold text-white/70">
                     <span>Volume</span>
                     <span className="text-white/60">{volume}%</span>
                   </div>
                   <Slider value={[volume]} onValueChange={(v) => updateVolume(v[0] ?? volume)} onValueCommit={(v) => handleVolumeCommit(v[0] ?? volume)} max={100} step={1} className="cursor-pointer" />
                 </PopoverContent>
               </Popover>

               <button onClick={toggleFavorite} className={`flex h-8 w-8 cursor-pointer items-center justify-center rounded-full border transition-colors ${isFavorite ? "border-pink-400/60 bg-pink-500/30 text-pink-400" : "border-white/10 bg-white/10 text-white hover:bg-white/20"}`} title={isFavorite ? "Remove from favorites" : "Add to favorites"}>
                 <Heart className="h-3.5 w-3.5" fill={isFavorite ? "currentColor" : "none"} />
               </button>

               <div className="flex-1" />

               <button onClick={toggleFullscreen} className={`flex h-8 w-8 cursor-pointer items-center justify-center rounded-full border transition-colors ${isFullscreen ? "border-purple-400/60 bg-purple-500/30 text-purple-400" : "border-white/10 bg-white/10 text-white hover:bg-white/20"}`} title={isFullscreen ? "Exit fullscreen" : "Enter fullscreen"}>
                 {isFullscreen ? <Minimize2 className="h-3.5 w-3.5" /> : <Maximize2 className="h-3.5 w-3.5" />}
               </button>

               {selectedChannel && (
                 <Link href={`/tv/${selectedChannel.id}`} className="flex h-8 items-center justify-center gap-0 rounded-full border border-white/10 bg-white/10 px-3 text-[10px] font-semibold tracking-wide text-white uppercase transition-colors hover:bg-white/20" title="Open channel page">
       ...
       <Dialog open={aboutDialogOpen} onOpenChange={setAboutDialogOpen}>
         <DialogContent className="sm:max-w-106.25">
           <DialogHeader>
             ...
             <DialogDescription className="mt-2 text-left">Watch IPTV streams (HLS) with favorites, volume control, and fullscreen support. Browse channels by category or country.</DialogDescription>
           </DialogHeader>
